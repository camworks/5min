...
with st.sidebar:
    ...
    과매수_기준 = st.slider("과매수 선 (70)", 50, 95, 70)
    과매도_기준 = st.slider("과매도 선 (30)", 5, 50, 30)
...
if not 실행_스위치:
    st.info("사이드바에서 '프로그램 시작'을 눌러주세요.")
...
while 실행_스위치:
    ...
    if df is not None:
        ...
        with 메인_영역.container():
            ...
            # EMA 선 설정 (색상 및 굵기)
            차트.add_trace(go.Scatter(x=df['timestamp'], y=df['EMA120'], line=dict(color='white', width=1), name='이평 120 (흰색)'))
            차트.add_trace(go.Scatter(x=df['timestamp'], y=df['EMA240'], line=dict(color='red', width=2), name='이평 240 (적색)'))
            차트.add_trace(go.Scatter(x=df['timestamp'], y=df['EMA600'], line=dict(color='purple', width=3), name='이평 600 (보라색)'))
            차트.add_trace(go.Scatter(x=df['timestamp'], y=df['EMA2400'], line=dict(color='yellow', width=4), name='이평 2400 (노란색)'))
